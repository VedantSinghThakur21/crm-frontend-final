/*
  ...
  1. New Tables
    - `equipment`
      - Equipment inventory with details and availability
    - `operators`
      - Operator profiles and specializations
    - `jobs`
...
INSERT INTO public.operators (user_id, name, email, phone, specialization)
SELECT 
  id,
  user_metadata->>'name',
  email,
  ...
  CASE 
    WHEN user_metadata->>'role' = 'operator' THEN 'Tower Crane'
    ELSE 'Multiple'
  END
FROM auth.users
WHERE user_metadata->>'role' = 'operator';