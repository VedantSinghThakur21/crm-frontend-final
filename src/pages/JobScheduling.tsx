...
import { getJobs, getAllEquipment, getAllOperators, createJob, updateJobStatus, deleteJob } from '../services/jobService';
...
export function JobScheduling() {
  ...
  const handleCreateJob = async () => {
    ...
    if (conflicts.length > 0) {
      showToast('Cannot schedule due to conflicts', 'error');
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="space-y-6">
          <Select
            label={<RequiredLabel>Select Customer</RequiredLabel>}
            ...
            error={!formData.leadId && isSubmitting ? "Customer is required" : undefined}
          />
          ...
          <div className="grid grid-cols-2 gap-4">
            <Select
              ...
              error={!formData.equipmentId && isSubmitting ? "Equipment is required" : undefined}
            />

            <Select
              label={<RequiredLabel>Operator</RequiredLabel>}
              ...
              error={!formData.operatorId && isSubmitting ? "Operator is required" : undefined}
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input
              type="datetime-local"
              label={<RequiredLabel>Start Date & Time</RequiredLabel>}
              value={formData.startDate}
              onChange={(e) => setFormData(prev => ({ ...prev, startDate: e.target.value }))}
              error={!formData.startDate && isSubmitting ? "Start date is required" : undefined}
            />

            <Input
              type="datetime-local"
              label={<RequiredLabel>End Date & Time</RequiredLabel>}
              value={formData.endDate}
              onChange={(e) => setFormData(prev => ({ ...prev, endDate: e.target.value }))}
              error={!formData.endDate && isSubmitting ? "End date is required" : undefined}
            />
          </div>
          ...
          <Input
            ...
            error={!formData.location && isSubmitting ? "Location is required" : undefined}
          />
